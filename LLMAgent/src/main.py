...
# inputs
def prompt_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, LLMAgent)
...
def currentLLM_input_callback(iop_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, LLMAgent)
        agent_object.currentLLMI = value
        # add code here if needed
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.input_create("currentLLM", igs.STRING_T, None)

    igs.output_create("response", igs.STRING_T, None)

    igs.observe_input("prompt", prompt_input_callback, agent)
    igs.observe_input("currentLLM", currentLLM_input_callback, agent)
